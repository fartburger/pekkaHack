...
import com.fartburger.fartcheat.util.math.Matrix4x4;
...
public class Renderer {
    ...
    public static class R3D {
        ...
        public static void renderFadingBlocks(MatrixStack stack) {
            ...
            Camera camera = FCRMain.client.getEntityRenderDispatcher().camera;
        ...
        static Matrix4f setupStack(MatrixStack stack) {
            Camera camera = FCRMain.client.gameRenderer.getCamera();
            Vec3d camPos = camera.getPos();
        ...
        private static void genericAABBRender(VertexFormat.DrawMode mode, VertexFormat format, Supplier<ShaderProgram> shader, MatrixStack stack, Vec3d start, Vec3d dimensions, Color color, RenderAction action) {
    ...
    public static class R2D {
        ...
        public static Vec2f renderTooltip(MatrixStack stack, Color color, double arrowX, double arrowY, double width, double height, boolean renderUpsideDown) {
            double centerX = FCRMain.client.getWindow().getScaledWidth() / 2d;
            /*
            left:
            *           /\
            * --------------
        ...
        public static void beginScissor(double x, double y, double endX, double endY) {
            ...
            float mulScale = (float) FCRMain.client.getWindow().getScaleFactor();
            int invertedY = (int) ((FCRMain.client.getWindow().getScaledHeight() - (y + height)) * mulScale);
        ...
        public static void renderLoadingSpinner(MatrixStack stack, float alpha, double x, double y, double rad, double width, double segments) {
            ...
            stack.multiply(new Quaternionf(0, 0, rot * 360f, 1));
            double segments1 = MathHelper.clamp(segments, 2, 90);
        ...
        public static Vec3d getScreenSpaceCoordinate(Vec3d pos, MatrixStack stack) {
            ...
            Vector4f transformedCoordinates = new Vector4f((float) x, (float) y, (float) z, 1.f).mul(matrix);

            int displayHeight = FCRMain.client.getWindow().getHeight();
            Vector3f screenCoords = new Vector3f();
            int[] viewport = new int[4];
            GL11.glGetIntegerv(GL11.GL_VIEWPORT, viewport);
            Matrix4f matrix4x4Proj = new Matrix4f(RenderSystem.getProjectionMatrix());//no more joml :)
            Matrix4f matrix4x4Model = new Matrix4f(RenderSystem.getModelViewMatrix());//but I do the math myself now :( (heck math)
            matrix4x4Proj.mul(matrix4x4Model).project(transformedCoordinates.x(), transformedCoordinates.y(), transformedCoordinates.z(), viewport, screenCoords);
            return new Vec3d(screenCoords.x / FCRMain.client.getWindow().getScaleFactor(),
                    (displayHeight - screenCoords.y) / FCRMain.client.getWindow().getScaleFactor(),
                    screenCoords.z);
        }
