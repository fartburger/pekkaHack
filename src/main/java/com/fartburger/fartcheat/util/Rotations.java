...
public class Rotations {
    ...
    public static void lookAtPositionSmoothServerSide(Vec3d target, double laziness) {
        double delX = target.x - Objects.requireNonNull(FCRMain.client.player).getX();
        double delZ = target.z - FCRMain.client.player.getZ();
        double delY = target.y - (FCRMain.client.player.getY() + FCRMain.client.player.getEyeHeight(FCRMain.client.player.getPose()));
    ...
    public static void lookAtPositionSmooth(Vec3d target, double laziness) {
        ...
        double degTan = -Math.toDegrees(Math.atan2(delY, sqrt1));
            // setting yaw
            delta = MathHelper.wrapDegrees(required - FCRMain.client.player.getYaw());
            speed = Math.abs(delta / laziness);
            add = speed * (delta >= 0 ? 1 : -1);
            if ((add >= 0 && add > delta) || (add < 0 && add < delta)) {
                add = delta;
            }
            FCRMain.client.player.setYaw(FCRMain.client.player.getYaw() + (float) add);

            // setting pitch
            required = degTan;
            delta = MathHelper.wrapDegrees(required - FCRMain.client.player.getPitch());
            speed = Math.abs(delta / laziness);
            add = speed * (delta >= 0 ? 1 : -1);
            if ((add >= 0 && add > delta) || (add < 0 && add < delta)) {
                add = delta;
            }
            FCRMain.client.player.setPitch(FCRMain.client.player.getPitch() + (float) add);

    }

    public static Vec2f getPitchYaw(Vec3d targetV3) {
        return getPitchYawFromOtherEntity(Objects.requireNonNull(FCRMain.client.player).getEyePos(), targetV3);
    }
