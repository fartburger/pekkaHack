...
import com.fartburger.fartcheat.util.render.ClipStack;
...
public class TextField extends Element {
    ...
    public TextField(double x, double y, double width, double height, String suggestion, double rad) {
    ...
    public boolean keyPressed(int key, int mods) {
        ...
        if (control && key == GLFW.GLFW_KEY_C) {
            if (cursor != selectionStart || cursor != selectionEnd) {
                FCRMain.client.keyboard.setClipboard(text.substring(selectionStart, selectionEnd));
            }
            return true;
    ...
    public boolean onKeyRepeated(int key, int mods) {
        ...
        if (control && key == GLFW.GLFW_KEY_V) {
            ...
            String preText = text;
            String clipboard = FCRMain.client.keyboard.getClipboard();
            int addedChars = 0;
